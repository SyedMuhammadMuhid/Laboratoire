...
class DatabaseService {
  ...
  final CollectionReference _collectionReference_userdata =
      FirebaseFirestore.instance.collection('UserData');
  final CollectionReference _collectionReference_points = FirebaseFirestore
      .instance
      .collection('UserData')
      .doc(uid_constant)
      .collection('Points');
  ...
  Future UpdateUserData(
    ...
    try {
      return await _collectionReference_userdata.doc(uid).set({
        ...
        'End_date': Start_date.add(Duration(days: Total_duration))
      });
    } catch (e) {
      return null;
    }
...
// user data stream to pre occupy the edit form and then edit and detect changes
  Stream<UserData> get userData {
    return _collectionReference_userdata
        .doc(uid)
        .snapshots()
  ...
  Future UpdatePointsSingleUpdate(
      int index_of_todo, String index_of_day, int value) async {
    index_of_todo = index_of_todo + 1;

    try {
      return await _collectionReference_points
          .doc(index_of_day)
          .update({'ToDo$index_of_todo': value});
    } catch (e) {
      return null;
    }
  }

  Future UpdatePointsTotal(
      String name_of_entry, String index_of_day, int value) async {
    try {
      return await _collectionReference_points
          .doc(index_of_day)
          .update({'$name_of_entry': value});
    } catch (e) {
      return null;
    }
  }
